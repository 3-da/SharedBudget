...
describe('CreatePersonalExpenseDto', () => {
    ...
    describe('amount', () => {
        ...
        it('should reject a sub-cent amount (more than 2 decimal places)', async () => {
            const dto = plainToInstance(CreatePersonalExpenseDto, { ...validMonthly, amount: 50.005 });
            const errors = await validate(dto);
            const amountError = errors.find((e) => e.property === 'amount');
            expect(amountError).toBeDefined();
        ...
        it('should accept an amount with a single decimal place', async () => {
            const dto = plainToInstance(CreatePersonalExpenseDto, { ...validMonthly, amount: 50.1 });
            const errors = await validate(dto);
            expect(errors.length).toBe(0);
        });

        it('should accept an amount with exactly 2 decimal places', async () => {
            const dto = plainToInstance(CreatePersonalExpenseDto, { ...validMonthly, amount: 50.99 });
            const errors = await validate(dto);
            expect(errors.length).toBe(0);
        });
    });
