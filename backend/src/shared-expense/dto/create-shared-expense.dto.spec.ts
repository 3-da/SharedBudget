...
describe('CreateSharedExpenseDto', () => {
    ...
    it('should reject a sub-cent amount (more than 2 decimal places)', async () => {
        const dto = plainToInstance(CreateSharedExpenseDto, { ...validDto, amount: 50.005 });
        const errors = await validate(dto);
    ...
    it('should accept an amount with a single decimal place', async () => {
        const dto = plainToInstance(CreateSharedExpenseDto, { ...validDto, amount: 50.1 });
        const errors = await validate(dto);
        expect(errors.length).toBe(0);
    });

    it('should accept an amount with exactly 2 decimal places', async () => {
        const dto = plainToInstance(CreateSharedExpenseDto, { ...validDto, amount: 50.99 });
        const errors = await validate(dto);
        expect(errors.length).toBe(0);
    });
    //#endregion
